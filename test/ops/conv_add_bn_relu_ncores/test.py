...
def conv_add_bn_relu(num, hin, win, cin, cout, kh, kw, sh=1, sw=1, dh=1, dw=1, pt=0, pb=0, pl=0, pr=0):
    ...
    vsadd = np.random.random(shape_output).astype('float16') * 2 - 1
...
def test(num, params, defs, ncores=8):
    ...
    os.system(f"make DEFS='{defines} -DCORENUMS={ncores} {defs}' run SIM={simulator} NUM={num} NCORES={ncores} >build/{num}/test.log 2>&1")
