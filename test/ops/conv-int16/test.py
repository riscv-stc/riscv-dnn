...
opt_levels = {"im2col14":"-O2 -DNLOOPS=14 -D__RVM__", "im2col24":"-O2 -DNLOOPS=24 -D__RVM__"}
...
def conv(num, hin, win, cin, cout, kh, kw, sh=1, sw=1, dh=1, dw=1, pt=0, pb=0, pl=0, pr=0):
    ...
    vs1 = np.random.randint(-1, 2, size=shape_input, dtype=np.int16)
    vs2 = np.random.randint(-1, 2, size=shape_weight, dtype=np.int16)
    tf_pad = [[0, 0], [pt, pb], [pl, pr], [0, 0]]
    vd = tf.nn.conv2d(vs1.astype('int32'), vs2.astype('int32'), [1, sh, sw, 1], tf_pad, data_format='NHWC', dilations=[1, dh, dw, 1])
    vd = vd.numpy().astype('int16')
