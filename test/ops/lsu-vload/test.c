...
int main(int argc, char **argv)
{
    ...
    for(int i = 0; i < NLOOPS; i++) {
        ...
        asm volatile("vle16.v v2, (%[rs1])"
                  :
                  :[rs1]"r"(src1Data+(i+1)*64));
        asm volatile("vle16.v v3, (%[rs1])"
                  :
                  :[rs1]"r"(src1Data+(i+2)*64));
        asm volatile("vle16.v v4, (%[rs1])"
                  :
                  :[rs1]"r"(src1Data+(i+3)*64));
        asm volatile("vle16.v v5, (%[rs1])"
                  :
                  :[rs1]"r"(src1Data+(i+4)*64));
        asm volatile("vle16.v v6, (%[rs1])"
                  :
                  :[rs1]"r"(src1Data+(i+5)*64));
        asm volatile("vle16.v v7, (%[rs1])"
                  :
                  :[rs1]"r"(src1Data+(i+6)*64));
        asm volatile("vle16.v v8, (%[rs1])"
                  :
                  :[rs1]"r"(src1Data+(i+7)*64));
        asm volatile("vle16.v v9, (%[rs1])"
                  :
                  :[rs1]"r"(src1Data+(i+8)*64));
        asm volatile("vle16.v v10, (%[rs1])"
                  :
                  :[rs1]"r"(src1Data+(i+9)*64));
    }
    PERF_END();
