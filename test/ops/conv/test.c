...
{
    printf("Begin\n");
    printf("%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", HIN, WIN, CIN, COUT, PAD_TOP, PAD_BOTTOM, PAD_LEFT, PAD_RIGHT, KH, KW, STRIDE_H, STRIDE_W, DILATION_H, DILATION_W);
    config_conv(sst, HIN, WIN, CIN, COUT, PAD_TOP, PAD_BOTTOM, PAD_LEFT, PAD_RIGHT, KH, KW, STRIDE_H, STRIDE_W, DILATION_H, DILATION_W);

    tensor_new_3d_with_stride(srcMat, HIN, WIN, CIN, sizeof(float16_t), srcData, CIN*sizeof(float16_t));
    tensor_new_4d_with_stride(weightMat, KH, KW, CIN, COUT, sizeof(float16_t), weightData, COUT*sizeof(float16_t));
    tensor_new_3d_with_stride(dstMat, HOUT, WOUT, COUT, sizeof(float16_t), &dstData, COUT*sizeof(float16_t));
