...
opt_levels = {"rvm":"-O2 -D__RVM__ -DFP16_ACC16", "rvm":"-O2 -D__RVM__ -DFP16_ACC16" }
...
def matmul(num, m, k, n):
    ...
    vs2 = np.random.random((n, k)).astype('float16') * 2 - 1
    vd = np.matmul(vs1, np.transpose(vs2), dtype=np.float16)
...
def test(num, params, defs, ncores=8):
    ...
    os.system(f"make DEFS='-DM={m} -DK={k} -DN={n} -DCORENUMS={ncores} {defs}' run SIM={simulator} NUM={num} NCORES={ncores} >build/{num}/test.log 2>&1")
