...
static inline int conv_ncores_cout(Tensor *dst, Tensor *src, Tensor *weight,
                                   Config *ss, int ncores, int pid) {
  ...
  for (int i = 0; i < kh; i++) {
    for (int j = 0; j < kw; j++) {
      for (int n = 0; n < ncores; n++) {
        ...
        tensor_new_3d_with_stride(_srcMat, hin, win, part_cin, dataSize, _src,
                                  stride_src);

        int cout_idx = pid;

        char *_weight = pweight + cout_idx * part_cout * dataSize +
                        cin_idx * part_cin * stride_weight;

        tensor_new_4d_with_stride(_weightMat, kh, kw, part_cin, part_cout,
                                  dataSize, _weight, stride_weight);

        char *_dst = pdst + cout_idx * part_cout * dataSize;

        tensor_new_3d_with_stride(_dstMat, hout, wout, part_cout, dataSize,
                                  _dst, stride_dst);

        config_conv(sst, hin, win, part_cin, part_cout, pad_t, pad_b, pad_l,
...
static inline int conv_ncores_hout(Tensor *dst, Tensor *src, Tensor *weight,
                                   Config *ss, int ncores, int pid) {
  int stride_h = ss->stride_h;
  int stride_w = ss->stride_w;
...
static inline int conv_ncores_cin(Tensor *dst, Tensor *src, Tensor *weight,
                                  Config *ss, int ncores, int pid) {
  int stride_h = ss->stride_h;
  int stride_w = ss->stride_w;
  ...
  for (int n = 0; n < ncores; n++) {
    ...
    for (int i = 0; i < kh; i++) {
      for (int j = 0; j < kw; j++) {
        ...
        char *_weight =
            pweight + cout_idx * part_cout * dataSize +
            (i * kw * ncores + j * ncores + pid) * part_cin * stride_weight;
...
static inline int conv_im2col_ncores(Tensor *dst, Tensor *src, Tensor *weight,
                                     Config *ss, int ncores, int pid) {
