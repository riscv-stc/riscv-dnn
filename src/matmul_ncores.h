...
static inline int matmul_n_ncores(Tensor *dst, Tensor *src1, Tensor *src2, int ncores)
{
    int m = src1->shape[0];
    int k = src1->shape[1];
    int n = src2->shape[1];
    ...
    for (int i = 0; i < ncores; ++i) {
        ...
        tensor_new_2d_with_stride(_src2Mat, part_k, part_n, dataSize, _src2, stride_s2);
...
static inline int matmul_m_ncores(Tensor *dst, Tensor *src1, Tensor *src2, int ncores)
{
    int m = src1->shape[0];
    int k = src1->shape[1];
    int n = src2->shape[1];
    ...
    for (int i = 0; i < ncores; ++i) {
        for (int j = 0; j < ncores; ++j) {
            ...
            tensor_new_2d_with_stride(_src1Mat, part_m, part_k, dataSize, _src1, stride_s1)
            ...
            tensor_new_2d_with_stride(_dstMat, part_m, part_n, dataSize, _dst, stride_d);
...
static inline int matmul_k_ncores(Tensor *dst, Tensor *src1, Tensor *src2, int ncores)
{
    int m = src1->shape[0];
    int k = src1->shape[1];
    int n = src2->shape[1];
...
static inline int matmul_ncores(Tensor *dst, Tensor *src1, Tensor *src2, int ncores)
{
