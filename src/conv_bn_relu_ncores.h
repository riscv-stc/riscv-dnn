...
static inline int conv_bn_relu_ncores_hout_cout(Tensor *dst, Tensor *src,
                                                Tensor *weight, Tensor *alpha,
  ...
  for (int i = 0; i < ncores; ++i) {
    ...
    conv_bn_relu_rvm(&_dstMat, &_srcMat, &_weightMat, &_alphaMat, &_betaMat,
                     &_sst);
  }
...
static inline int conv_bn_relu_ncores_hout(Tensor *dst, Tensor *src,
                                           Tensor *weight, Tensor *alpha,
                                           Tensor *beta, Config *ss, int ncores,
                                           int pid) {
  int stride_h = ss->stride_h;
  int stride_w = ss->stride_w;
  ...
  tensor_new_3d_with_stride(_dstMat, part_hout, wout, cout, dataSize, _dst,
                            stride_dst);

  config_conv(_sst, part_hin, win, cin, cout, _pad_t, _pad_b, pad_l, pad_r, kh,
              kw, stride_h, stride_w, dilation_h, dilation_w);
...
static inline int conv_bn_relu_ncores_cout(Tensor *dst, Tensor *src,
                                           Tensor *weight, Tensor *alpha,
                                           Tensor *beta, Config *ss, int ncores,
                                           int pid) {
  int stride_h = ss->stride_h;
  ...
  tensor_new_3d_with_stride(_dstMat, hout, wout, part_cout, dataSize, _dst,
                            stride_dst);

  config_conv(_sst, hin, win, cin, part_cout, pad_t, pad_b, pad_l, pad_r, kh,
              kw, stride_h, stride_w, dilation_h, dilation_w);
