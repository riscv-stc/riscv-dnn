...
/*
    ...
    β: [c]
*/
static int batchnorm2(Tensor *dst, Tensor *src, Tensor *mean, Tensor *variance,
                      Tensor *gam, Tensor *beta, float16_t epsilon) {
  int h = src->shape[0];
  int w = src->shape[1];
  int c = src->shape[2];
...
#ifdef DEBUG
  printf("%f, %f, %f, %f, %f, %f\n", (float)*psrc, (float)*pmean, (float)*pvar,
         (float)*pgam, (float)*pbeta, (float)epsilon);
#endif
...
/*
    ...
    β: [c]
*/
static inline int batchnorm(Tensor *dst, Tensor *src, Tensor *alpha,
                            Tensor *beta) {
  int h = src->shape[0];
  int w = src->shape[1];
  int c = src->shape[2];
