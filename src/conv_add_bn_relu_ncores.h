...
static inline int conv_add_bn_relu_ncores_hout(Tensor *dst, Tensor *addout, Tensor *src, Tensor *weight, Tensor *addsrc, Tensor *alpha, Tensor *beta, Config *ss, int ncores)
{
    ...
    for (int i = 0; i < ncores; ++i) {
      ...
      char *_addsrc = paddsrc + hout_idx * part_hout * wout * stride_addsrc  + cout_idx * part_cout * dataSize;
      char *_addout = paddout + hout_idx * part_hout * wout * stride_addout  + cout_idx * part_cout * dataSize;
      tensor_new_3d_with_stride(_addsrcMat, part_hout, wout, part_cout, dataSize, _addsrc, stride_addsrc);
      tensor_new_3d_with_stride(_addoutMat, part_hout, wout, part_cout, dataSize, _addout, stride_addout);

      conv_add_bn_relu_rvm(&_dstMat, &_addoutMat, &_srcMat, &_weightMat, &_addsrcMat, &_alphaMat, &_betaMat, &_sst);
    }
    
    return 0;
}

static inline int conv_add_bn_relu_ncores(Tensor *dst, Tensor *addout, Tensor *src, Tensor *weight, Tensor *addsrc, Tensor *alpha, Tensor *beta, Config *ss, int ncores)
{
  conv_add_bn_relu_ncores_hout(dst, addout, src, weight, addsrc, alpha, beta, ss, ncores);
}
