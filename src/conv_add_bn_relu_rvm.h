...
static inline int conv_add_bn_relu_rvm(void *dst, void *addout, void *src, void *addsrc, void *weight,  void *alpha, void *beta, Config *ss)
{
    ...
    for (int i = 0; i < m; i+=tilem) {
      ...
      for (int j = 0; j < n; j+=tilen) {
        ...
        // add
        mfloat16_t acc1 = mlc_m(paddsrc+i*stride_addsrc/dataSize+j, stride_addsrc);
        acc0 = mfadd_mm(acc0, acc1);
        msc_m(acc0, paddout+i*stride_addout/dataSize+j, stride_addout);
        
        // batchnormal
        mfloat16_t mbeta = mlc_m(palpha + j, stride_addout);
        mfloat16_t malpha = mlc_m(pbeta + j, stride_addout);
        mbeta = mbccr_m(mbeta);
      ...
      for (int j = 0; j < n; j+=tilen) {
        ...
        for (int skh = 0; skh < kh; skh++) {
          ...
          for (int skw = 0; skw < kw; skw++) {
            ...
            for (int skc = 0; skc < cin;  skc+=tilek) {
                tilek = msettilek(cin-skc);
                mfloat16_t tr0 = mlc_m(_psrc1+skc, stride_s1);
                mfloat16_t tr1 = mlc_m(_psrc2+skc*stride_s2/dataSize, stride_s2);
                acc0 = mfma_mm(acc0, tr0, tr1);
            }
          }
        }
        ...
        for (int skh = 0; skh < kh; skh++) {
          ...
          for (int skw = 0; skw < kw; skw++) {
            ...
            for (int skc = 0; skc < cin;  skc+=tilek) {
                tilek = msettilek(cin-skc);
                mfloat16_t tr0 = mlufa_m(_psrc1+skc, stride_s1);
                mfloat16_t tr1 = mlbe16_m(_psrc2+skc*stride_s2/dataSize, stride_s2);
...
static inline int conv_add_bn_relu_rvm_batch8(void *dst, void *addout, void *src, void *addsrc, void *weight, void *alpha, void *beta, Config *ss)
{
    ...
    for (int i = 0; i < m; i+=tilem) {
      ...
      for (int j = 0; j < n; j+=tilen) {
        ...
        mfloat16_t acc0 = mlce16_m(_paddsrc, stride_addsrc);
        _paddsrc += addsrcSize;
        mfloat16_t acc1 = mlce16_m(_paddsrc, stride_addsrc);
        _paddsrc += addsrcSize;
        mfloat16_t acc2 = mlce16_m(_paddsrc, stride_addsrc);
        _paddsrc += addsrcSize;
        mfloat16_t acc3 = mlce16_m(_paddsrc, stride_addsrc);
        _paddsrc += addsrcSize;
        mfloat16_t acc4 = mlce16_m(_paddsrc, stride_addsrc);
        _paddsrc += addsrcSize;
        mfloat16_t acc5 = mlce16_m(_paddsrc, stride_addsrc);
        _paddsrc += addsrcSize;
        mfloat16_t acc6 = mlce16_m(_paddsrc, stride_addsrc);
        _paddsrc += addsrcSize;
        mfloat16_t acc7 = mlce16_m(_paddsrc, stride_addsrc);
        ...
        // batch 0
        msc_m(acc0, _paddout, stride_addout);
        acc0 = mfmul_mm(acc0, malpha);
        acc0 = mfadd_mm(acc0, mbeta);
        acc0 = mfmax_mm(acc0, zero);
        ...
        _pdst += outSize;
        _paddout += addoutSize;
        msc_m(acc1, _paddout, stride_addout);
        ...
        _pdst += outSize;
        _paddout += addoutSize;
        msc_m(acc2, _paddout, stride_addout);
        ...
        _pdst += outSize;
        _paddout += addoutSize;
        msc_m(acc3, _paddout, stride_addout);
        ...
        _pdst += outSize;
        _paddout += addoutSize;
        msc_m(acc4, _paddout, stride_addout);
        ...
        _pdst += outSize;
        _paddout += addoutSize;
        msc_m(acc5, _paddout, stride_addout);
        ...
        _pdst += outSize;
        _paddout += addoutSize;
        msc_m(acc6, _paddout, stride_addout);
        ...
        _pdst += outSize;
        _paddout += addoutSize;
        msc_m(acc7, _paddout, stride_addout);
...
static inline int conv_add_bn_relu_rvm_batch4(void *dst, void *addout, void *src, void *addsrc, void *weight, void *alpha, void *beta, Config *ss)
{
    ...
    for (int i = 0; i < m; i+=tilem) {
      ...
      for (int j = 0; j < n; j+=tilen) {
        tilen = msettilen(n-j);
        float16_t *_paddsrc = paddsrc + i*stride_addsrc/dataSize+j;
        mfloat16_t acc0 = mlc_m(_paddsrc, stride_addsrc);
        _paddsrc += addsrcSize;
        mfloat16_t acc1 = mlc_m(_paddsrc, stride_addsrc);
        _paddsrc += addsrcSize;
        mfloat16_t acc2 = mlc_m(_paddsrc, stride_addsrc);
        _paddsrc += addsrcSize;
        mfloat16_t acc3 = mlc_m(_paddsrc, stride_addsrc);
