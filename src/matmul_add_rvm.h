...
static inline int matmul_add_matrix_batch16(void *dst, void *src1, void *src2, ConfigMatmul *ss, int srcSize, int dstSize)
{
    ...
    for(int i = 0; i < m; i += tile_m) {
        ...
        for (int j = 0; j < n; j += tile_n) {
            ...
            mfloat16_t acc0 = mlc_m(_pdst, stride_d*dataSize);
            _pdst += dstSize;
            mfloat16_t acc1 = mlc_m(_pdst, stride_d*dataSize);
            _pdst += dstSize;
            mfloat16_t acc2 = mlc_m(_pdst, stride_d*dataSize);
            _pdst += dstSize;
            mfloat16_t acc3 = mlc_m(_pdst, stride_d*dataSize);
            _pdst += dstSize;
            mfloat16_t acc4 = mlc_m(_pdst, stride_d*dataSize);
            _pdst += dstSize;
            mfloat16_t acc5 = mlc_m(_pdst, stride_d*dataSize);
            _pdst += dstSize;
            mfloat16_t acc6 = mlc_m(_pdst, stride_d*dataSize);
            _pdst += dstSize;
            mfloat16_t acc7 = mlc_m(_pdst, stride_d*dataSize);
            _pdst += dstSize;
            mfloat16_t acc8 = mlc_m(_pdst, stride_d*dataSize);
            _pdst += dstSize;
            mfloat16_t acc9 = mlc_m(_pdst, stride_d*dataSize);
            _pdst += dstSize;
            mfloat16_t acc10 = mlc_m(_pdst, stride_d*dataSize);
            _pdst += dstSize;
            mfloat16_t acc11 = mlc_m(_pdst, stride_d*dataSize);
            _pdst += dstSize;
            mfloat16_t acc12 = mlc_m(_pdst, stride_d*dataSize);
            _pdst += dstSize;
            mfloat16_t acc13 = mlc_m(_pdst, stride_d*dataSize);
            _pdst += dstSize;
            mfloat16_t acc14 = mlc_m(_pdst, stride_d*dataSize);
            _pdst += dstSize;
            mfloat16_t acc15 = mlc_m(_pdst, stride_d*dataSize);
