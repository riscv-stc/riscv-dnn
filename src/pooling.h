...
static inline int avgpool(Tensor *dst, Tensor *src, Config *ss) {
  ...
  for (int i = 0; i < hout; i++) {
    ...
    for (int j = 0; j < wout; j++) {
      ...
      int srcOffset =
          (sh0 + hStart - pad_t) * win * cin + (sw0 + wStart - pad_l) * cin;
...
static inline int avgpool_mean(Tensor *dst, Tensor *src, Config *ss) {
  ...
  for (int kc = 0; kc < cin; kc += vl) {
    ...
    for (int i = 0; i < hin; i++) {
      for (int j = 0; j < win; j++) {
        vfloat16m8_t _data = vle16_v_f16m8(
            psrc + i * win * stride_src / 2 + j * stride_src / 2 + kc, vl);
        _sum = vfadd_vv_f16m8(_sum, _data, vl);
        // asm volatile("vle16.v v8, (%[rs1])"
        //             :
        //             : [rs1]"r"(psrc + i*win*stride_src/2 + j*stride_src/2 +
        //             kc));
      ...
      for (int kc = 0; kc < cin; kc += vl) { // complete vlmax point one time
        ...
        for (int m = 0; m < kh; m++) {
          ...
          for (int n = 0; n < kw; n++) {
            ...
            unsigned srcOffset =
                (sy - pad_t) * win * cin + (sx - pad_l) * cin + kc;
